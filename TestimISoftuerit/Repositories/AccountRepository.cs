    ...
    public class AccountRepository : IUserAccount
    {
        ...
        public async Task<GeneralResponse> CreateAccount(UserDTO userDTO)
        {
            ...
            var newUser = new ApplicationUser
            {
                ...
                EmailConfirmationTokenCreatedAt = DateTime.UtcNow // Set token creation time
            };
        ...
        public async Task<ApplicationUser?> GetUserByRefreshToken(string refreshToken)
        {
            ...
            return await userManager.Users
                ...
                .FirstOrDefaultAsync(u => u.RefreshTokens.Any(rt => rt.Token == refreshToken && rt.Expired > DateTime.Now));
        }

        public async Task<LoginResponse> RefreshToken(string token)
        {
            if (string.IsNullOrEmpty(token))
                return new LoginResponse(false, null, "Token is required");

            var user = await GetUserByRefreshToken(token);
            if (user == null)
                return new LoginResponse(false, null, "Invalid or expired token");
            ...
            return new LoginResponse(true, newToken, "Token refreshed successfully");
        }
